...
func TestEvidencePoolUpdate(t *testing.T) {
	...
	block := types.MakeBlock(height+1, []types.Tx{}, nil, lastExtCommit.ToCommit(), []types.Evidence{ev})
...
func initializeBlockStore(db dbm.DB, state sm.State, valAddr []byte) (*store.BlockStore, error) {
	...
	for i := int64(1); i <= state.LastBlockHeight; i++ {
		...
		block := state.MakeBlock(i, test.MakeNTxs(i, 1), nil, lastCommit.ToCommit(), nil, state.Validators.Proposer.Address)
