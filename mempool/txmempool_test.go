...
func newMempoolWithAppTxMpool(cc proxy.ClientCreator) (*TxMempool, cleanupFunc) {
	conf := test.ResetTestRoot("mempool_test")

	mp, cu := newMempoolWithAppAndConfigTxMpool(cc, conf)
	return mp, cu
}

func newMempoolWithAppAndConfigTxMpool(cc proxy.ClientCreator, cfg *config.Config) (*TxMempool, cleanupFunc) {
	...
	mp := NewTxMempool(log.TestingLogger(), cfg.Mempool, appConnMem, NewTestPeerEvictor())
...
type TestPeerEvictor struct {
	evicting map[p2p.ID]struct{}
}

func NewTestPeerEvictor() *TestPeerEvictor {
	return &TestPeerEvictor{evicting: map[p2p.ID]struct{}{}}
}

func (e *TestPeerEvictor) IsEvicted(peerID p2p.ID) bool {
	_, ok := e.evicting[peerID]
	return ok
}

func (e *TestPeerEvictor) Errored(peerID p2p.ID, err error) {
...
func TestTxMempool_Size(t *testing.T) {
	...
	require.Equal(t, int64(5390), txmp.SizeBytes())
	...
	require.Equal(t, int64(2700), txmp.SizeBytes())
}
...
func TestTxMempool_ReapMaxBytesMaxGas(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		reapedTxs := txmp.ReapMaxBytesMaxGas(-1, 1)
		ensurePrioritized(reapedTxs)
		require.Equal(t, len(tTxs), txmp.Size())
...
func TestTxMempool_Prioritization(t *testing.T) {
	...
	for i := range txsCopy {
		require.NoError(t, txmp.CheckTx(txsCopy[i], nil, TxInfo{SenderID: peerID}))
	}
...
func TestTxMempool_PendingStoreSize(t *testing.T) {
	...
	require.NoError(t, txmp.CheckTx([]byte(fmt.Sprintf("evm-sender=%s=%d=%d", address1, 1, 1)), nil, TxInfo{SenderID: peerID}))
	err := txmp.CheckTx([]byte(fmt.Sprintf("evm-sender=%s=%d=%d", address1, 1, 2)), nil, TxInfo{SenderID: peerID})
	require.Error(t, err)
...
func TestTxMempool_CheckTxPostCheckError(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			txmp := setup(t, client, 0, WithPostCheckTxMpool(postCheckFn))
			rng := rand.New(rand.NewSource(time.Now().UnixNano()))
			tx := make([]byte, txmp.config.MaxTxBytes-1)
			_, err := rng.Read(tx)
			require.NoError(t, err)
			...
			if testCase.err == nil {
				...
				err = txmp.CheckTx(tx, callback, TxInfo{SenderID: 0})
				fmt.Print(err.Error())
			}
		})
	}
...
func TestTxMempool_FailedCheckTxCount(t *testing.T) {
	...
	// bad tx
	require.NoError(t, txmp.CheckTx(tx, callback, TxInfo{SenderID: 0, SenderP2PID: "sender"}))
