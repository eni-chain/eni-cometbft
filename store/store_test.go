...
func TestBlockStoreSaveLoadBlock(t *testing.T) {
	...
	block := state.MakeBlock(bs.Height()+1, txs, nil, new(types.Commit), nil, state.Validators.GetProposer().Address)
...
func TestLoadBlockPart(t *testing.T) {
	...
	block := state.MakeBlock(height, nil, nil, new(types.Commit), nil, state.Validators.GetProposer().Address)
	partSet, err := block.MakePartSet(types.BlockPartSizeBytes)
	require.NoError(t, err)
